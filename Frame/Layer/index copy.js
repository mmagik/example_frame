...
class Layer extends React.Component {
    ...
    checkResourceIsReady(layer) {
        ...
        return {
            ...
            processingProgress: resource ? resource.processingProgress : null
        }
    }
    ...
    render() {
        ...
        // console.log("resource--", getResource(layerId, layerState.items[0].resourceId))

        // ? можно в сам DataItem добавить метод для получения прогресса для таймлайна.
        // там же и будет проверка isResourceWillTransfered()
        // другими словами, нужно сокрыть вызов isResourceWillTransfered, 
            // а тут только получать нужный объект с прогрессом, или инфой, что объект не нуждается в трансфере
        // ? Для получения прогресса вызвать ф-ию getTransferProgress() у resourceDataItem
        // # При обновлении прогресса в DataItem нужно триггерить обновление слоя тут, чтобы показывать прогресс
        // ? Учесть что ресурса вовсе может не быть, например, для текста


        console.log(
            "getTransferProgress",
            getResource(layerId, layerState.items[0].resourceId) 
                ? getResource(layerId, layerState.items[0].resourceId).resourceDataItem.getTransferProgress()
                : "?"
        )

        // if (this.props.active) {
        //     console.log(this.props.layer.startSeconds)
        // }


        const { loading, width } = this
        const isLoading = loading != null
        const showLeftResizer = width > 10
        const style = this.layerStyle

        // console.log("loading", loading)

        return (
            <div className={className} style={style} data-layerid={layerId} data-index={this.props.index} data-actiontype="drag" data-isloading={isLoading} onPointerEnter={this.onLayerPointerEnter} onPointerLeave={this.onLayerPointerLeave}>
                {showLeftResizer && <div className="t-layer-resize t-layer-resize-left" data-actiontype="resize" data-resizertype="left" data-isloading={isLoading}></div>}
                <div className="t-layer-resize t-layer-resize-right" data-actiontype="resize" data-resizertype="right" data-isloading={isLoading}></div>
                ...
                <div className="t-layer-preview-wrapper">
                    {/* Layer preview will be here */}
                    {isLoading 
                        ? <Loading loadedPercent={loading} />
                        : layerState 
                            ? <LayerPreview 
